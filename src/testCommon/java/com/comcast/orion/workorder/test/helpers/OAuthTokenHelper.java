package com.comcast.orion.workorder.test.helpers;
...
public class OAuthTokenHelper {
	...
	public static String getToken() {
		try {
			...
			claims.setIssuer("PingFederate"); // who creates the token and signs
												// it
			claims.setExpirationTimeMinutesInTheFuture(1000); // time when the
																// token will
																// expire (10
																// minutes from
																// now)
			claims.setClaim("client_id", "orion_dev"); // additional
														// claims/attributes
														// about the subject can
														// be added
			claims.setStringListClaim("scope", newArrayList("orion:orion-dev")); // additional
																					// claims/attributes
																					// about
																					// the
																					// subject
